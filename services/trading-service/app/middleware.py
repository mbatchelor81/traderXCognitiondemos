"""
Middleware for the trading-service: tenant pinning, correlation ids and
request timing.
"""
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if requested_tenant and requested_tenant != TENANT_ID:
            logger.warning(
                ...
                extra={"requested_tenant": requested_tenant},
            )
            return JSONResponse(
                status_code=403,
                content={
...
class CorrelationIdMiddleware(BaseHTTPMiddleware):
    """Propagates a correlation id through the request and the response."""
