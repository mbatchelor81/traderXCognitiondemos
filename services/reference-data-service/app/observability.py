"""
Prometheus metrics and OpenTelemetry tracing for reference-data-service
(TARGET_ARCHITECTURE_CONSTRAINTS.md §10).

Metrics are served on ``/metrics`` and, because a single ALB fronts every
service, also on ``/svc/reference-data/metrics``. ``/svc/reference-data/health`` mirrors ``/health``
