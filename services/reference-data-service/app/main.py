"""Reference Data Service FastAPI application."""
import logging
import signal
import sys
...
def create_app() -> FastAPI:
    app = FastAPI(title="Reference Data Service", version="1.0.0", description="Provides stock ticker reference data")

    app.add_middleware(CORSMiddleware, allow_origins=CORS_ORIGINS, allow_credentials=True, allow_methods=["*"], allow_headers=["*"])
    app.add_middleware(TenantMiddleware)
    app.include_router(router, tags=["Stocks"])
    ...
    def on_startup():
        ...
        logger.info("Reference Data Service started", extra={"tenant_id": TENANT_ID, "service": SERVICE_NAME})
