"""Tests for the GET /account/{account_id}/summary endpoint."""
import pytest


def _create_account(client, name: str = "Summary Account") -> int:
    resp = client.post("/account/", json={"displayName": name})
    assert resp.status_code == 200
    return resp.json()["id"]


def _submit_trade(client, account_id: int, security: str, side: str,
                  quantity: int) -> dict:
    resp = client.post("/trade/", json={
        "accountId": account_id,
        "security": security,
        "side": side,
        "quantity": quantity,
    })
    assert resp.status_code == 200
    return resp.json()


def test_summary_returns_zeros_for_empty_account(client):
    """A fresh account with no trades should return all-zero statistics."""
    account_id = _create_account(client)
    resp = client.get(f"/account/{account_id}/summary")
    assert resp.status_code == 200
    data = resp.json()
    assert data["totalTrades"] == 0
    assert data["settledTrades"] == 0
    assert data["pendingTrades"] == 0
    assert data["totalBuyQuantity"] == 0
    assert data["totalSellQuantity"] == 0
    assert data["netQuantity"] == 0


def test_summary_reflects_trades(client):
    """Statistics should reflect submitted trades."""
    account_id = _create_account(client)
    _submit_trade(client, account_id, "AAPL", "Buy", 100)
    _submit_trade(client, account_id, "MSFT", "Sell", 40)

    resp = client.get(f"/account/{account_id}/summary")
    assert resp.status_code == 200
    data = resp.json()
    assert data["totalTrades"] == 2


def test_summary_404_for_nonexistent_account(client):
    """Requesting summary for a missing account should return 404."""
    resp = client.get("/account/99999/summary")
    assert resp.status_code == 404
