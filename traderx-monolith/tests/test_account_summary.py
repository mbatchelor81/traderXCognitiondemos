...
def test_account_summary_empty(client):
    """Summary for an account with no trades returns zero stats."""
...
def test_account_summary_with_trades(client):
    """Summary reflects trades after they are submitted."""
    ...
    assert isinstance(data["settledTrades"], int)
    assert isinstance(data["pendingTrades"], int)
    assert isinstance(data["netQuantity"], int)
