...
def get_account_summary(account_id: int, request: Request,
                        db: Session = Depends(get_db)):
    """
    Get aggregated trading statistics for an account.

    Leverages the existing cross-domain aggregation in trade_processor
    (`get_account_portfolio_summary`) and returns the statistics block:
    total trades, settled/pending counts, buy/sell quantities, net quantity.
    """
    tenant_id = get_tenant_from_request(request)
    ...
    return {
        ...
        "displayName": summary["account"].get("displayName"),
